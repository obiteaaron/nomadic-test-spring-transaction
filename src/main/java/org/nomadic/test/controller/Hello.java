...
public class Hello {
    ...
    public Object testDb1(@PathVariable("action") String action, @PathVariable("name") String name, @PathVariable(value = "id") Long id) {
        if (ACTION_INSERT.equals(action)) {
            wordService.insert(name);
            return ACTION_INSERT + " " + name;
        } else if (ACTION_UPDATE.equals(action)) {
            wordService.update(id, name);
            return ACTION_UPDATE + " " + id + " " + name;
        } else {
    ...
    public Object testDb2(@PathVariable("action") String action, @PathVariable("name") String name, @PathVariable(value = "id") Long id) {
        if (ACTION_INSERT.equals(action)) {
